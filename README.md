...
- Create folders:
   ...
   - prep directory: includes files from dowsing, packmol-memgen, leap, system.pdb, system.prmtop, system.inpcrd, system_HMR.prmtop, sample run_amber.sh, sample Eq, Min, Heat, Prod files
  
- Edit run_amber.sh
   - Confirm that the sequence of commands is correct, and that the reference files (-ref) are set correctly, to either Min_3.rst or the preceding step rst file
   - Set the run time to 8 hours to start
   - Decide queue (savio3_gpu or savio4_gpu)
   - Partition, qos, nodes per GPU, and gres may need to change depending on queue (reference table in the savio website). Also may need to exclude nodes.
   - We run on GPUs. 1 node, 1 task, 2 CPUs per task = 2 CPU cores per sim run
...
- Copy the Min, Heat, Eq, Prod sample files to every run subdirectory
- Add symlinks to the .prmtop and .inpcrd files, to each run subdirectory
   `ln -s fullpathtoprepdirectory/system_HMR.prmtop system.prmtop`
   `ln -s fullpathtoprepdirectory/system.incprd system.inpcrd`
- Submit jobs
  - submit 1 job for Min steps (8 hour timestep)
...
- Check number of ns/us: `cat mdinfo | grep TIME`
 ...
 - Download trajectory
	- change .nc to .dcd for the output file	
 	- Use the auto_reimage.in reference file provided
 	- Check the most recent timestep for the sim, ex: Prod_30.rst 
 	- Open in vim, comment out any "trajin" lines, starting with Prod_31.rst  
		v (visual mode); shift-v; use down arrow to go up to the last trajin; type ":s/^/#"; hit Enter
	- For every run directory, do the following:
   ```
   source activate AmberTools21
   cpptraj -i auto_reimage.in
   ```
- Edit .prmtop file (one per sim type)
topology and trajectory files must match -> must grab .pdb file and match it
go to prep directory
take .pdb file and get rid of the waters in it (? .pdb or .prmtop)
make a file called strip_wat.in (vim stripwater.in makes the file)
"parm 4xaq_4of_14sb_tip3_HMR.prmtop
parmstrip :WAT
parmwrite out protein_nowat.prmtop
go
exit"
replace input.prmtop file with your own correct file names

cpptraj -i stripwater.in
protein_nowater.prmtop file has been created in the prep dir

- Download the .dcd files and .prmtop file to local computer, make sure to rename the .dcd files to refer to the runs they originate from after downloading, otherwise you will rewrite each time
 - Load pdb, prmtop, into VMD
 - Change representations to something meaningful to you
 - Load one dcd at a time, visualize
 - You can add new molecules, or you can delete every frame after frame 1 (the pdb crystal strxr) and then load in a new dcd trajectory each time.
--- 
### Add to tutorial:  

Techniques
- Setting up Anton sims from Savio sims
- parametrizing a UAA
- full pipeline for performing an analysis, hbonds script added in public folder  
...
More thoughts:
- coordinates may be slightly shifted, or no?
- to delete extra charges and balance the system, you can just delete ions from the membrane pdb file
- membrane systems often have bad clashes of lipid chains that need resolving via the minimization steps
...
- explain HMR - one liner

- Justin’s tutorial mentions using 3D-RISM to add waters to the system. We used Maestro’s add waters function → what is the mathematical diff
